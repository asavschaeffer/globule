...
class ThoughtPalette(Vertical):
    ...
    async def load_initial_content(self, topic: str):
        ...
        if not clusters or not any(clusters.values()):
            tree.root.add_leaf("No initial thoughts found for topic - try searching above")
            return
    ...
    def action_save_module(self):
        ...
        if node and hasattr(node, 'data') and node.data:
            ...
            try:
                ...
                self.app.update_status(f"❌ Save failed: {str(e)[:50]}...")
        else:
            tree.add("⚠️ Select a module with content first")
            self.app.update_status("⚠️ No module selected for saving")
